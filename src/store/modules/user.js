...
import {userLogin, sendSMSMsg, findPwd, userInfo, userVerify} from '../../api/user'
...
const actions = {
  async login ({commit}, {param, router, redirect}) {
    const {data, code, statusCode} = await userLogin(param)
    const {mobile, step} = param
    if (step !== 1 && code === 'suss') {
      commit(types.USER_LOGIN, {data, mobile})
    }
    return await {data, code, statusCode}
  },
  ...
  async sendSmsCode ({commit}, {param}) {
    const {data, code, statusCode} = await sendSMSMsg(param)
    commit(types.SEND_SMS_CODE, {data})
    return await {data, code, statusCode}
  },
  async findPassword ({commit}, {param}) {
    const {data, code} = await findPwd(param)
